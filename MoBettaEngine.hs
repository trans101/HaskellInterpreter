...
retrieveEnv name = do
  ...
  where
    varNotFound name = error $ "VAR NOT FOUND ERROR: Identifier \"" ++ name ++ "\" not defined."
...
boolCalc (Reln cOp expr1 expr2) = do
  c1 <- intCalc expr1
  c2 <- intCalc expr2
  return ((fromMaybe(error "ERROR: Relational Operation") (lookup cOp relnOps))c1 c2)

boolCalc (BBin op expr1 expr2) = do
  c1 <- boolCalc expr1
  c2 <- boolCalc expr2
  return ((fromMaybe(error "ERROR: Binary Operation")(lookup op bBinOps)) c1 c2)

boolCalc (BUn op expr) = do
  c <- boolCalc expr
  return ((fromMaybe(error "ERROR: Unary Operation")(lookup op bUnOps)) c)

intCalc :: AExpr -> IntCalc
intCalc (Var v) = retrieveEnv v
intCalc (IntConst val) = return val
intCalc (ABin op expr1 expr2) = do
  -- pass the first expression into a variable
  c1 <- intCalc expr1
  -- pass the 2nd expression into a variable
  c2 <- intCalc expr2
  -- throw error if user tries to divide by zero
  if c2 == 0 && op == Div then error "ERROR: Division by Zero" else
    return ((fromMaybe(error "ERROR: Calculating Integers")(lookup op aBinOps)) c1 c2)

intCalc (AUn op expr) = do
  c <- intCalc expr
  return ((fromMaybe(error "ERROR: Calculating int Operation")(lookup op aUnOps)) c)
